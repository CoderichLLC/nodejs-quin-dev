...
module.exports = (factory) => {
  describe('Quin Integration', () => {
    describe('Create', () => {
      test('Person', async () => {
        const model = 'Person';

        const $richard = { age: 40, name: 'Richard', status: 'alive', state: 'NJ', emailAddress: 'rich@coderich.com', network: 'network', strip: 'mall', multiLang: 'lang', 'multiLang.en': 'en', 'multiLang.es': 'es' };
        richard = await factory({ model }).save($richard);
        expect(richard).toEqual({ id: expect.anything(), ...$richard });

        const $christie = { name: 'Christie', emailAddress: 'christie@gmail.com', friends: [richard.id], telephone: 1112223333, network: 'network', nonsense: 'nonsense' };
        christie = await factory({ model }).save($christie);
        expect(christie).toEqual({ id: expect.anything(), ...$christie });
      });

      test('Book', async () => {
        const model = 'Book';

        const $mobyDick = { name: 'moby dick', price: 9.99, bids: [1.99, 1.20, 5.00], bestSeller: true, author: richard.id };
        mobyDick = await factory({ model }).save($mobyDick);
        expect(mobyDick).toEqual({ id: expect.anything(), ...$mobyDick });

        const $healthBook = { name: 'Health and Wellness', bids: [5.00, 9.00, 12.50], price: '29.99', author: christie.id };
        healthBook = await factory({ model }).save($healthBook);
        expect(healthBook).toEqual({ id: expect.anything(), ...$healthBook });
      });
